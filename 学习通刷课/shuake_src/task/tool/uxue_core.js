/*
 * uXueScript core.js v2.0.2 —— 来源：https://github.com/unraous/uxuescript
 * 许可证：GPL-3.0-only（原样嵌入，未做修改）
 * 本文件是自包含单文件脚本：既可由本工具经 Selenium execute_script 注入
 * （无后端模式 + Python 伪造的 Tauri IPC 桥），也可直接粘贴浏览器控制台运行。
 * 原产品契约：无 Tauri 后端时使用默认配置并跳过 Quiz 任务点。
 */
